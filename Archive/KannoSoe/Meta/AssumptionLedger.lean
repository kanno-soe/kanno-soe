/-
================================================================================
  KannoSoe.Meta.AssumptionLedger
...
import KannoSoe.Meta.AxiomAudit
...
def assumptionLedger : List AssumptionEntry := [
  ...
  { «section» := .asserted
    ...
    anchors := [
      ...
      downWitness ``SelfLineWitness.selfLine_ksmdOwnershipFace_self
    ] },
  { «section» := .asserted
    number := 7
    title := "Dukkha and Bull 10 are reading-relative"
    statement := "`ClenchMismatch` and its share covariation are grid-derived. `KsmdDukkha` adds the supplied mark: the structure is derived, the suffering is supplied. Bull 10 likewise quantifies over `SentientTag` under a reading; with the constant-false reading its marketplace is empty and the predicate is unsatisfiable."
    anchors := [
      downProof ``Grid.ClenchMismatch,
      downProof ``Grid.KsmdDukkha,
      downProof ``Grid.clenchMismatch_of_ksmdDukkha,
      downProof ``Grid.KsmdBullTen,
      downProof ``Grid.not_ksmdBullTen_allInsentient
    ] },
  { «section» := .declined
    number := 1
  ...
  { «section» := .declined
    ...
    anchors := [
      downProof ``Grid.DirectedConvention.KsmdEffectiveOccurrence,
      downProof ``Grid.DirectedConvention.KsmdEffectivenessEnacted,
      downProof ``Grid.DirectedConvention.not_effectivenessEnacted_of_undelivered,
      downProof ``Grid.DirectedConvention.KsmdFullyEnlightened,
      downProof ``Grid.DirectedConvention.KsmdNoNescience,
      downProof ``Grid.DirectedConvention.KsmdFullyEnlightenedEnacted,
      ...
      downProof ``Grid.DirectedConvention.BeingConvention.GridConvention.ksmd_effective_occurrence_voice_assertable,
      downProof ``Grid.DirectedConvention.BeingConvention.GridConvention.ksmd_standing_effectiveness_voice_displayable
    ] },
  { «section» := .declined
    number := 7
  ...
  { «section» := .convenience
    ...
    statement := "`KsmdMismatchGrade` lives in `Doctrines`, so this Signature module does not import it; the Signature-side checked fact is that `share` is the only contribution value exported by a weld."
    anchors := [
      sigProof ``Grid.share,
      sigProof ``Grid.share_eq_grade_check,
      downProof ``Grid.KsmdMismatchGrade,
      downProof ``Grid.ksmdMismatchGrade_eq_share
    ] },
  { «section» := .convenience
    number := 4
