...
import KannoSoe.Consequences.ContentRows
...
theorem relabel_deliveredTo_iff (deed reception : G.Weld) :
  ...
  unfold DirectedConvention.DeliveredTo KannoSoe.DeliveredTo
...
@[simp]
theorem map_ksmdMismatchGrade (w : G.Weld) :
    (G.map f).KsmdMismatchGrade w = f.toFun (G.KsmdMismatchGrade w) :=
  rfl
...
theorem map_ksmdDukkha_iff (S : SentienceReading G) (w : G.Weld) :
    (G.map f).KsmdDukkha (S.displayMap f) w ↔ G.KsmdDukkha S w := by
  constructor
  · intro h
...
theorem map_ksmdBullSeven_iff (b : Designatum) :
    (G.map f).KsmdBullSeven b ↔ G.KsmdBullSeven b := by
  constructor
...
theorem map_ksmdBullTen_iff {Macro : Type}
    (S : SentienceReading G)
    (κ : DirectedConvention.BeingConvention.BeingCoarsening G Macro)
    (b : Designatum) :
    (G.map f).KsmdBullTen (S.displayMap f) (κ.displayMap f) b ↔
      G.KsmdBullTen S κ b := by
  constructor
  · intro h
...
theorem map_ksmdEffectiveTerminus_reflect
    {b : Designatum} (h : KsmdEffectiveTerminus (G.map f) b) :
    KsmdEffectiveTerminus G b := by
  constructor
...
/-- Preservation of the universally quantified faith closure needs the target
    ...
    `CoverageNegative.ksmdEffectiveTerminus_needs_coverage` shows the hypothesis
    is needed. -/
theorem map_ksmdEffectiveTerminus_of_surjective
    (hsurj : ∀ b' : Contrib', ∃ a : Contrib, f.toFun a = b')
    {b : Designatum} (h : KsmdEffectiveTerminus G b) :
    KsmdEffectiveTerminus (G.map f) b := by
  constructor
...
def KsmdPathClaim.map (c : KsmdPathClaim G) : KsmdPathClaim (G.map f) :=
...
theorem map_ksmdPathClaim_holds_iff
    (t : Tier G) (c : KsmdPathClaim G) :
    (ksmdPathClaimLanguage (G.map f)).Holds (Tier.map f t)
        (KsmdPathClaim.map G f c) ↔
      (ksmdPathClaimLanguage G).Holds t c := by
  cases t with
  | floor =>
      exact Iff.rfl
  | actTime _ =>
      simpa [ksmdPathClaimLanguage, KsmdPathClaim.map, Tier.map] using
...
/-- With target-carrier coverage, effective termination is display-invariant
    outright. -/
theorem map_ksmdEffectiveTerminus_iff
    (hsurj : ∀ b' : Contrib', ∃ a : Contrib, f.toFun a = b')
    (b : Designatum) :
    KsmdEffectiveTerminus (G.map f) b ↔ KsmdEffectiveTerminus G b :=
  ⟨fun h => map_ksmdEffectiveTerminus_reflect G f h,
    fun h => map_ksmdEffectiveTerminus_of_surjective G f hsurj h⟩
...
theorem map_effectiveTerminus_eq
    ...
    Faith (KsmdEffectiveTerminus (G.map f) b) =
      Faith (KsmdEffectiveTerminus G b) := by
  rw [propext (map_ksmdEffectiveTerminus_iff G f hsurj b)]

theorem map_ksmdAversionContext_iff
    (before : Config Contrib) (reception : G.Weld) :
    KsmdAversionContext (G.map f) (before.map f) reception ↔
      KsmdAversionContext G before reception := by
  constructor
  · intro h
    refine
      { liveBefore := ?_
