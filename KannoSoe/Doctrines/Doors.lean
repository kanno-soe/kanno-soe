...
def KsmdDefiledFalsehood {L : ClaimLanguage G} (sr : SpeechReading G L)
...
theorem not_defiledFalsehood_of_atBot {L : ClaimLanguage G}
    ...
    (hbot : AtBot (G.share u.weld)) : ¬ KsmdDefiledFalsehood sr u := by
  intro hfalse
...
abbrev KsmdDefiledFalsehood (G : CoreReadings Designatum Contrib)
    {L : Grid.ClaimLanguage G} (sr : Grid.SpeechReading G L) :=
  Grid.KsmdDefiledFalsehood sr
