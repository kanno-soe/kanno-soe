/-
================================================================================
  KannoSoe.Identification.Commentary
...
Names beginning `Ksmd` mark identifiers that assert a system-POV reading: what
...
conditional vocabulary such as `KsmdMismatchGrade`, `KsmdDukkha`,
`KsmdAversionContext`, `KsmdEffectiveTerminus`, and the tariki/jiriki line
readings. Doctrine names are carried by module names and commentary headings;
checked reading identifiers use the single `Ksmd` marker.
...
`ksmdSuddenArrival_witness`, `ksmdGradualArrival_witness`,
...
`ksmdSuddenArrival_witness` gives the doctrine-facing one-step form with
actuality, `ksmdGradualArrival_witness` gives the register-clock staged form,
...
recall as reception: `recall_ksmdOwnershipFace` names the delivered trace,
`falseMemory_ksmdVacuousOwnershipFace` names the unfilled second place, and
...
pole-class. `thirdArrival_not_ksmdMismatchLive` reuses the terminus-response
...
`ksmdPathOught_conditional`, `no_final_level_of_errorFree`, and
...
`directionVoid_of_surjective` and `map_ksmdEffectiveTerminus_of_surjective`:
...
quietness through all three doors. `KsmdDefiledFalsehood` is the speech-door
...
`KsmdMismatchGrade` is definitionally `share`; this is the formal honesty clause
for mismatch-talk as covariation rather than a second measure.
`ClenchMismatch` is the actual live-share structure. `KsmdDukkha S` adds the
supplied mark, so `clenchMismatch_of_ksmdDukkha` forgets the phenomenal reading
...
`KsmdAversionContext` treats sraddha reception per call: it packages a live
prior tendency and an actual clench-mismatch reception, not a stored faith
possession. `KsmdEffectiveTerminus` deliberately has two conjuncts: terminus
typing and universal shortfall closure for delivered deeds. The physician
simile belongs exactly there: the grid can prove `ksmdPathOught_conditional`,
...
`FettersNegative.total_cut_with_actual_occurrence_not_ksmdEffectiveTerminus`.
...
`KsmdNoNescience` requires positive own-act-time truth for every pole-share
...
`KsmdNoDelusion` comparison. The converse fails:
...
occurrences. `KsmdFullyEnlightenedEnacted` names the further samyaksambuddha
rung by adding `KsmdEffectivenessEnacted` and a tied
`KsmdFaithfulSpeechOccurrence`; the old untied act-time witness has disappeared.
...
`KsmdEthicsStance` bundles factivity and faith in the two-obscurations object and
requires admitted fidelity records to arise from speech productions.
`KsmdEthicalCode` is the fourth-truth ought generalized over such testimony,
still an implication type only. The is-ought work is receiver-side: testimony
supplies the is, while the receiver's live `KsmdAversionContext` supplies the
...
responsive terminus need not be `KsmdEffectiveTerminus`.
The opposite regime face is checked by
`ksmdEffectiveTerminus_of_responsiveTerminus_of_undelivered`: with no delivered
...
`KsmdEffectiveOccurrence`: an actual pole-deed landing as a share-drop against
a live prior tendency. The standing universal survives only as display and
direct-path hypothesis; `KsmdEffectivenessEnacted` and
...
`KsmdFullyEnlightenedEnacted`. The separate standing faith-object is the
two-obscurations `KsmdFullyEnlightened` bundle; the enacted top adds its named
...
while flipping `KsmdEffectiveTerminus`.
...
a fact about `respondsTo`, not a Ksmd reading. `landing_call_in_modality` is
...
cells. `KsmdSuddenArrival` is the actual one-step share-drop to the pole-class,
and `ksmdSuddenArrival_witness` reuses the clock-grid possibility shape.
`KsmdGradualArrival` folds a `ShareDropRun` through `rePitchRun`, with
`ksmdGradualArrival_witness` on `registerClockGrid`. `rate_invisible_to_config`
...
`KsmdBullSeven`, probe-constancy plus a live self-pole index. The theorem
...
cross-fiber delivery predicate `KsmdBullTen S`. The stronger `StrongKsmdBullTen S`
is named and shelved. `not_ksmdBullTen_allInsentient` owns the consequence that
the marketplace is empty under the constant-false reading. The pratyekabuddha
countermodel shows Bull 9 without Bull 10.

The Five Ranks are data plus reading pins; `kenChuTo_implies_ksmdBullTen` names
...
The door-typed `KsmdSravakaArhat` is speech-and-mind quiet; `KsmdVasana` is a live
...
`KsmdEffectiveTerminus`, terminus plus universal shortfall closure, and
it enters the direct śraddhā route as hypothesis. The shushō-ittō face is now
explicit as `KsmdEffectiveOccurrence`; the standing rung is a descriptive
...
not imply `KsmdNoNescience`. The two-obscurations `KsmdFullyEnlightened` bundle
...
`OrthogonalityNegative.ksmdEffectiveTerminus_stronger_than_terminus` shows that
even effective termination fails. Samyaksambuddha is instead
`KsmdFullyEnlightenedEnacted`, where a delivered effective occurrence and a
...
`KsmdResolveAttenuation` gives once-return positive content without adding a cut
class: a resolve-class share-drop run can be strict while still stopping short
of the pole. `KsmdSerialFactorRegime` is the matching "usually in order"
...
`ksmdIrreversibleRegime_conditional`, parallel in voice to the sraddha
...
delivered, and the two agent tags are equal or unequal. `KsmdJirikiLine` and
`KsmdTarikiLine` carry the system-POV reading of those neutral facts.
...
`ksmdReachBack_filled_either_regime` records the one-act-grammar point: once an
...
`KannoSoe/Exposition/Gen/Glossary.lean`; the paper-facing table lives at
...
so `not_ksmdEffectiveTerminus_of_icchantika` gives the honest agent-role bar:
this run cannot seat the icchantika as the enlightened agent, because
`KsmdEffectiveTerminus` includes terminus typing.

The receiver-side result points the other way.
`aversionContext_of_icchantika_reception` says an actual icchantika reception
with live prior tendency supplies the `KsmdAversionContext` antecedent, and
