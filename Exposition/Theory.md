...
| The ladder / its terminus | — | Emptiness-sickness (空病): seeking a fifth negation, the emptying itself frozen into a path-object instead of the seeking dropped. The absence is structural: there is no "completed ladder" claim constructor (`no_final_level_of_errorFree`). As a formal-model analogue of 空空, one priming already exhausts `Related`: priming again mints a further web-designatum but adds no relation-content on the once-primed image (`prime_related_exhausted_on_image`) |
...
The brackets assert no new relation among the designata already in view. Given the stated relation `a <--> b`, `[a <--> b]` designates that dependence-whole as the single designatum `ab`; thus `[a <--> b] <--> c` reads `ab <--> c`, and `a <--> [b <--> c]` reads `a <--> bc`.

The formal counterpart makes the limit of that claim precise. `contract E a b` extends the domain by a fresh designatum whose added elaboration clause is sourced only at that fresh point; the old clauses and their targets are lifted through the old-domain embedding. Old designata therefore cannot reach the fresh source, and `contract_related_iff` proves that contraction preserves `Related` on the common designata. In this limited formal sense, `===` can be read extensionally: the articulations induce the same `Related` there, not that they are numerically identical or that every feature of their `Reaches` graphs is the same. The source/target asymmetry matters. `prime E`, introduced below, instead makes the fresh point a target of every embedded old designatum and thereby saturates `Related`; contraction is conservative re-articulation, while priming is a tier-change.
...
When designating `a` in future, the designator may instead designate its primed image `a' := some a`, changing the mode of designation without withdrawing the lifted base clauses. Read at the floor, that primed designation carries the role-reading *with understanding all in each / each in all*: “understanding” names the added all-clause supplied in designation, not a sentience or nature possessed by `a'`. The floor-face offer does not replace act-time articulation (`exists_tier_noncollapse`).
