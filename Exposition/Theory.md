...
|---|---|---|---|
| **First-level (一重): being (有).** The cart, path, being, or another useful distinction is live. | A freeze occurs if being is treated as own-being (自性): the cart or another designation is treated as independently real. *(See the “Named being / floor” freeze in the taxonomy.)* | — | **Emptiness (空).** It denies own-being or final standing to the distinction without cancelling its conventional use. |
| **Second-level (二重): being-and-emptiness (有空).** The lower-level convention, its possible finalization, and emptiness as the correction of that finalization are all available as teaching. | A freeze occurs if emptiness is treated as a final absence standing over against being. The two-truths teaching is thereby recast as a two-item ontology. *(See the “Being / emptiness” freeze in the taxonomy.)* | A collapse occurs if the conventionally functioning cart is treated as the same target as the self-standing cart. A denial aimed at the latter is then extended to the former, yielding the erroneous conclusion: “Because no cart exists in itself, no cart is available for the task.” The same move can cancel a live practice or response. *(See the “Named being / floor” collapse in the taxonomy.)* | **Neither-being-nor-emptiness (非有非空).** It denies final standing to both being and emptiness without prohibiting their pedagogical use. |
...
The ladder re-indexes the same Grade-1 error-cells; it does not introduce new occupants. A freeze is available at the rung where a distinction is stated, whereas its collapse appears only on the next rung, after its ultimate truth has been stated. Thus **Named being / floor** supplies row 1’s freeze and row 2’s collapse, while **Being / emptiness (有空)** supplies row 2’s freeze and row 3’s collapse.
